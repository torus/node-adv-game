...
function scenes() {             // WIP
  ...
  // message array
  // state
  var name = name => scene => {scene.name = name; return masg => async stat => {}}
  var spot = spot => scene => {scene.spot = spot; return masg => async stat => {}}
  var desc = (...texts) => scene => mesg => {mesg.push(texts); return async stat => {}}
  var line = (actor, ...texts) => scene => mesg => {mesg.push(new Line(actor, texts)); return async stat => {}}
  var link = destScene => scene => {
    scene.linkedSceneSet[destScene] = true
    return mesg => async stat => {
  ...
  var when = (param, ...content) => scene => mesg => async stat => {}
  var not = param => scene => mesg => async stat => {}
  var action = param => scene => mesg => async stat => {}
  var label = text => scene => mesg => async stat => {}
  var after = (...content) => scene => mesg => async stat => {}
  var flagUp = param => scene => mesg => async stat => {}
  var flagDown = param => scene => mesg => async stat => {}
  var condition = param => scene => mesg => async stat => {}
    ...
    return mesg => async stat => {
      ...
      if (visited) {
        ...
        for (var i = 0; i < content.length; i ++) {
          await content[i](scene)(mesg)(stat)
        }
      }
    }
  }
