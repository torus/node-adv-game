...
router.get('/login/facebook', (ctx, next) => {
...
router.get('/sessions/:sessId', async (ctx, next) => {
  if (!ctx.isAuthenticated()) {
    ctx.redirect('/')
    return
  }
  ...
  ctx.body = make_body("Session",
                       E('h1', {}, 'Session'),
                       E('p', {}, "Hello, " + ctx.state.user.displayName
                         + "! (" + ctx.state.user.id + ")"),
                       E('p', {}, E('a', {href: ['/stage', session.id].join('/')}, "start game")),
...
router.get('/sessions', async (ctx, next) => {
  ...
  ctx.body = make_body('Playlist',
                       E('h1', {}, 'Continue Playing'),
                       E('ul', {},
...
router.post('/sessions', async ctx => {
  ...
  const sessId = await redis('incr', "maxid")
  const key = 'session:' + sessId
  const userId = ctx.state.user.id
...
var GameSession = function(webState, sessionId) {
  if (!sessionId)
...
async function setUserParam(session, param, value) {
  var key = "sessionParam:" + session.id;
...
async function addToUserParamSet(session, param, ...values) {
  if (values.length == 0)
    return
  var key = "sessionParamSet:" + param + ":" + session.id
...
async function loadGame(gameId) {
  ...
  var charactersElems = doc.filter(e => e.characters)
...
router.get('/stage/:sessId', async ctx => {
  ...
  const firstStage = game.scenes[0].stage

  ctx.redirect(['/stage', sessId, firstStage].join('/'))
})
...
router.get('/stage/:sessId/:stageid', async ctx => {
  ...
  var sessId = ctx.params.sessId
  var id = ctx.params.stageid
  var key = 'session:' + sessId
  const sess = JSON.parse(await redis('get', key))
  const gameId = sess.game

  try {
    var session = JSON.parse(await redis('get', key))
    var title = await stageTitle(session.game, id)
    var session = new GameSession(ctx.state, sessId)
