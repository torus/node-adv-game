...
function scenes() {             // WIP
  ...
  var link = destScene => scene => {
    ...
    return async (disp, acts, stat) => {
      ...
      console.log(destScene, sceneMap[destScene])
  ...
  var action = (...params) => scene => async (disp, acts, stat) => {
    acts.push(async stat => {
      var actObj = {}
      for (var i = 0; i < params.length; i ++) {
        await (await v(scene)(disp, acts, stat))(actObj)
      }
    })
  }
  var label = text => scene => async (disp, acts, stat) => actObj => {actObj.label = text}
  var after = (...content) => scene => async (disp, acts, stat) => async actObj => {
    actObj.handler = async ctx => {
      for (var i = 0; i < params.length; i ++) {
        await (await v(scene)(disp, acts, stat))(actObj)
      }
  ...
  var flagUp = param => scene => async (disp, acts, stat) => async actObj => {
    await setUserParam(stat, actObj.label, 1)
  }
