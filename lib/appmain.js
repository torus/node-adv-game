...
function scenes() {             // WIP
  var scene = (...args) => {
    ...
    self.desc = async stat => {
      ...
      for (var i = 0; i < procs.length; i ++) {
        var dest = procs[i](mesg)
        console.log("dest", dest)
        if (dest instanceof(Promise)) {
          dest = await dest
        }
        console.log("dest2", typeof(dest))
        if (dest instanceof(Function)) {
          console.log("func")
          dest = await dest(stat)
          console.log("dest3", dest)
        }
      }
  ...
  var name = name => s => {s.name = name}
  var spot = spot => s => {s.spot = spot}
  var desc = (...texts) => s => mesg => {mesg.push(texts)}
  var line = (actor, ...texts) => s => mesg => {mesg.push(new Line(actor, texts))}
  var link = scene => s => {}
  var when = (param, ...content) => s => mesg => {}
  var not = param => s => {}
  var action = param => s => {}
  var label = text => s => {}
  var after = (...content) => s => {}
  var flagUp = param => s => {}
  var flagDown = param => s => {}
  var condition = param => s => {}
...
function makeSpots() {
  ...
  var spotDescs = {}
  for (var s in scns) {
    var prev = spotDescs[scns[s].spot]
    if (!prev) {
