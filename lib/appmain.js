...
function scenes() {             // WIP
  ...
  var scene = (...args) => {
    ...
    self.desc = async stat => {
      var disp = []
      var acts = []
      for (var i = 0; i < procs.length; i ++) {
        await procs[i](disp, acts, stat)
      }
      return makeDisplayElements(disp)
    }
  ...
  var action = param => scene => async (disp, acts, stat) => {}
  var label = text => scene => async (disp, acts, stat) => {}
  var after = (...content) => scene => async (disp, acts, stat) => {}
  var flagUp = param => scene => async (disp, acts, stat) => {}
...
function makeSpots() {
  ...
  for (var s in scns) {
    ((s, prev) => {
      if (!prev) {
        spotDescs[scns[s].spot] = scns[s].desc
      } else {
        spotDescs[scns[s].spot] = async stat => {
          console.log("prev", prev)
          var prevResult = await prev(stat)
          var key = "currentScene-" + scns[s].spot
          if ((await getUserParam(stat, key)) == scns[s].name) {
            return [prevResult, await scns[s].desc(stat)]
          } else {
            return prevResult
          }
        }
      }
    }) (s, spotDescs[scns[s].spot])
  }

  for (var s in spots) {
    spots[s].desc = spotDescs[s]
  }
...
router.get('/spot/:id', async ctx => {
  ...
  try {
    ctx.body = make_body(
      ...
      E("ul", {id: "actionList"}, (await spotActions(id, ctx.state)).map(action => {
        return E("li", {},
