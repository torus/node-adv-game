...
var spots = {
  ...
  "grampa": {
    name: "おじいちゃんの家",
    desc: stat => {
      return "おじいちゃんの家につきましたが、おじいちゃんはどこにもいません。"
    },
    moves: []
  }
};
...
async function spotBody(id) {
  return spots[id].desc();
}
...
router.get('/spot/:id', async ctx => {
  ...
  ctx.body = make_body(
    title,
    E("h1", {}, title),
    E("p", {}, await spotBody(id)),
    E("ul", {}, (await spotMoveOptions(id)).map(spot => {
      return E("li", {}, E("a", {href: '/spot/' + spot.id}, spot.name));
    }))
  )
});
