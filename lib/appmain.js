  ...
  while (true) {
    ...
    try {
      await pgClient.query(
        ...
        [id, title, desc, new Date(), authorId, ver, url, false])
    } catch (e) {
      done = false
    }
...
router.get('/games/byAuthor/:authorId', async ctx => {
  ...
  const res = await (authorId == userId
                     ? pgClient.query('SELECT id, title FROM scenarios WHERE publisher = $1', [authorId])
...
router.get('/games/:gameid', async ctx => {
  ...
  ctx.body = await constructPage(
    ...
    scenario.title,
    E('h1', {}, scenario.title),
    E('p', {}, loc1('Published by ')(await getAuthorName(scenario.publisher))),
...
router.get('/sessions', async ctx => {
  ...
  const res = await pgClient.query(
    'SELECT se.id AS sid, sc.title, sc.version FROM sessions se JOIN scenarios sc ' +
      'ON (se.scenario = sc.id) WHERE se.user = $1',
    [userId])
...
//   const res = await pgClient.query(
//     'SELECT COUNT(*) AS c FROM session_states WHERE session = ? AND key= ? AND value = ?',
//     [session.id, param, value])
...
//   const res = await pgClient.query('DELETE FROM session_states WHERE session = ? AND key = ? AND value = ?',
//                                    [session.id, param])
// }
...
router.get('/users/:userId', async ctx => {
  ...
  const displayName = await getAuthorName(userId)
