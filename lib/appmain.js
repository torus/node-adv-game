...
async function scenes() {
  ...
  commands.name = name => scene => {scene.name = name; return async (disp, actions, stat) => {
    console.log("scene", name)
  }}
  commands.stage = stage => scene => {scene.stage = stage; return async (disp, actions, stat) => {}}
  commands.desc = (...texts) => scene => async (disp, actions, stat) => {disp.push(texts)}
  commands.speak = (actor, ...texts) => scene => async (disp, actions, stat) => {disp.push(new Speak(actor, texts))}
  ...
  commands.not = param => scene => async (disp, actions, stat) => {
  ...
  commands.label = text => scene => async (disp, actions, stat) => async action => {
    action.label = text
  }
  commands.after = (...content) => scene => async (disp, actions, stat) => async action => {
  ...
  commands.flagUp = param => scene => async (disp, actions, stat) => async action => {
    await setUserParam(stat, param, 1)
  }
  commands.flagDown = param => scene => async (disp, actions, stat) => {}
  commands.condition = param => scene => async (disp, actions, stat) => {}
  ...
  var conved = doc.map(elem => conv(elem))
  console.log(conved)
...
async function makeStages() {
  ...
  var scns = (await scenes()).reduce((acc, cur, idx, arr) => {acc[cur.name] = arr[idx]; return acc}, {})
  var stageDisplays = {}
