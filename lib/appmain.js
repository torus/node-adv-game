...
router.get('/stage/:sessId', async ctx => {
  ...
  if (res2.rowCount > 0) {
    ...
    await pgClient.query('INSERT INTO current_stages (session, stage) VALUES ($1, $2)',
                         [sessId, currentStage])
  }
...
router.get('/stage/:sessId/:stageid', async ctx => {
  ...
  await pgClient.query('UPDATE current_stages SET stage = $1 WHERE session = $2', [id, sessId])
