...
const redis = promisify(redisClient.send_command, redisClient);
...
var spots = {
  "home": {
    ...
    desc: async stat => {
      ...
      if (await redis('hget', [key, 'visited-home'])) {
        var mesg = [["お母さん「鍛冶屋さんに包丁を作ってもらって",
                     "それを海沿いの村のおじいちゃんの家にとどけてちょうだい」"]]
      } else {
        var mesg = [["ある日、おじいちゃんから手紙が届きました。",
                     "そこには、魚をさばくのに特別な包丁が必要なので、",
                     "私にそれを届けてほしい、と書いてありました。"],
                    ["お母さん「鍛冶屋さんに包丁を作ってもらって",
                     "それを海沿いの村のおじいちゃんの家にとどけてちょうだい」"]]
        await redis('hset', [key, 'visited-home', 1])
      }
      return mesg.map(lines => {
        return E('p', {}, lines.slice(1).reduce((part, line) => {
  ...
  "smith": {
    ...
    desc: async stat => {
      var key = "user:" + stat.user.id;
      if (await redis('hget', [key, 'visited-smith'])) {
        var mesg = [["おじさん「お嬢ちゃん、もし出来ればハガネユリをとってきてくれないかい？",
                     "あそこの山に流れる川の近くに生えているはずだ」"]]
      } else {
        var mesg = [["近くの鍛冶屋さんに来ました。",
                     "ここでいつもおじいちゃんの包丁を作ってもらうのです。",
                     "奥から鍛冶職人のおじさんが顔を出しました。"],
                    ["おじさん「やあよく来たね。",
                     "なに？　おじいちゃんのためにあの特別な包丁を作ってほしい？」"],
                    ["おじさん「まいったな、実はいまあの包丁を作るのに必要なハガネユリを切らしているんだ。",
                     "これは今の時期山に生えている花なんだがね、足の具合が悪くていけないのだよ。」"],
                    ["おじさん「お嬢ちゃん、もし出来ればハガネユリをとってきてくれないかい？",
                     "あそこの山に流れる川の近くに生えているはずだ」"]]
        await redis('hset', [key, 'visited-smith', 1])
      }
      return mesg.map(lines => {
        return E('p', {}, lines.slice(1).reduce((part, line) => {
          return part.concat([E('br'), line])
...
async function spotMoveOptions(id) {
  return spots[id].moves.map(id => {
...
router.get('/spot/:id', async ctx => {
  ...
  ctx.body = make_body(
    ...
    E("ul", {}, (await spotMoveOptions(id)).map(spot => {
