...
function scenes() {             // WIP
  ...
  var stage = stage => scene => {scene.stage = stage; return async (disp, acts, stat) => {}}
  var desc = (...texts) => scene => async (disp, acts, stat) => {disp.push(texts)}
  var speak = (actor, ...texts) => scene => async (disp, acts, stat) => {disp.push(new Speak(actor, texts))}
...
async function stageMoveOptions(id, stat) {
  ...
  return stagesFiltered.map(id => {
    return {id: id, name: stages[id].name}
  });
}
...
router.get('/stage/:id', async ctx => {
  ...
  try {
    var disp = await stages[id].display(ctx.state)
    ctx.body = make_body(
      title,
      E("h1", {}, title),
      disp[0],
      ...
      E("ul", {id: "moveList"}, (await stageMoveOptions(id, ctx.state)).map(stage => {
