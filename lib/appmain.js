...
function scenes() {             // WIP
  ...
  var name = name => scene => {scene.name = name; return async (disp, stat) => {}}
  var spot = spot => scene => {scene.spot = spot; return async (disp, stat) => {}}
  var desc = (...texts) => scene => async (disp, stat) => {disp.push(texts)}
  var line = (actor, ...texts) => scene => async (disp, stat) => {disp.push(new Line(actor, texts))}
  var link = destScene => scene => {
    scene.linkedSceneSet[destScene] = true
    return async (disp, stat) => {
  ...
  var when = (param, ...content) => scene => async (disp, stat) => {}
  var not = param => scene => async (disp, stat) => {}
  var action = param => scene => async (disp, stat) => {}
  var label = text => scene => async (disp, stat) => {}
  var after = (...content) => scene => async (disp, stat) => {}
  var flagUp = param => scene => async (disp, stat) => {}
  var flagDown = param => scene => async (disp, stat) => {}
  var condition = param => scene => async (disp, stat) => {}
    ...
    return async (disp, stat) => {
      ...
      if (visited) {
        ...
        for (var i = 0; i < content.length; i ++) {
          await content[i](scene)(disp, stat)
        }
      }
    }
  }
