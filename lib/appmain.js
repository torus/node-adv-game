...
function scenes() {             // WIP
  ...
  var scene = (...args) => {
    ...
    self.display = async stat => {
      ...
      for (var i = 0; i < procs.length; i ++) {
        await procs[i](disp, acts, stat)
      }
      return [makeDisplayElements(disp), acts]
    }
  ...
  var name = name => scene => {scene.name = name; return async (disp, acts, stat) => {}}
  var spot = spot => scene => {scene.spot = spot; return async (disp, acts, stat) => {}}
  var desc = (...texts) => scene => async (disp, acts, stat) => {disp.push(texts)}
  var line = (actor, ...texts) => scene => async (disp, acts, stat) => {disp.push(new Line(actor, texts))}
  var link = destScene => scene => {
    scene.linkedSceneSet[destScene] = true
    return async (disp, acts, stat) => {
      var spotsKey = "accessibleSpots:" + stat.user.id
      console.log("link", destScene, sceneMap[destScene])
      await redis('sadd', spotsKey, sceneMap[destScene].spot)
    }
  }
  var when = (param, ...content) => scene => async (disp, acts, stat) => {
    var val
    if (typeof(param) == "string") {
      val = await getUserParam(stat, param)
    } else {
      val = await param(scene)(disp, acts, stat)
    }

    console.log("when", param, val)
    if (val) {
      for (var i = 0; i < content.length; i ++) {
        await content[i](scene)(disp, acts, stat)
      }
    }
  }
  var not = param => scene => async (disp, acts, stat) => {
    var val = await getUserParam(stat, param)
    console.log("not", param, val)
    return (val == null) || (val == 0)
  }
  var action = (...params) => scene => async (disp, acts, stat) => {
    console.log("action")

    var actObj = {}
    for (var i = 0; i < params.length; i ++) {
      await (await params[i](scene)(disp, acts, stat))(actObj)
    }

    acts.push(actObj)
  }
  var label = text => scene => async (disp, acts, stat) => async actObj => {
    console.log("label", text)
    actObj.label = text
  }
  var after = (...content) => scene => async (disp, acts, stat) => async actObj => {
    var hndlid = actionHandlerCount++
    actObj.id = hndlid

    actionHandlers[hndlid] = async ctx => {
      for (var i = 0; i < content.length; i ++) {
        await (await content[i](scene)(disp, acts, stat))(actObj)
      }

      ctx.redirect('/spot/' + scene.spot)
    }
  }
  var flagUp = param => scene => async (disp, acts, stat) => async actObj => {
    console.log("flagUp", param)
    await setUserParam(stat, param, 1)
  }
  var flagDown = param => scene => async (disp, acts, stat) => {}
  var condition = param => scene => async (disp, acts, stat) => {}
  ...
  var intro = (...content) => scene => {
    ...
    return async (disp, acts, stat) => {
      ...
      console.log(visitedKey, visited)
      if (visited) {
      } else {
...
function makeSpots() {
  ...
  for (var s in scns) {
    ...
    ((scene, prevDisplay) => {
      if (!prevDisplay) {
        spotDisplays[spot] = scene.display
      } else {
        spotDisplays[spot] = async stat => {
          console.log("prevDisplay", prevDisplay)
          var prevDisplayResult = await prevDisplay(stat)
          var key = "currentScene-" + spot
          if ((await getUserParam(stat, key)) == scene.name) {
            return [prevDisplayResult, await scene.display(stat)]
          } else {
            return prevDisplayResult
          }
        }
      }
      console.log("spot", spot, scene)
      if (!spots[spot].moves) spots[spot].moves = []
      spots[spot].moves = spots[spot].moves.concat(Object.keys(scene.linkedSceneSet).map(destScene => {
        return scns[destScene].spot
      }))
      console.log("moves", spots[spot].moves)
    }) (scene, spotDisplays[spot])
  }

  for (var s in spots) {
    spots[s].display = spotDisplays[s]
    spots[s].moves = Object.keys(spots[s].moves.reduce((part, cur) => {part[cur] = true; return part}, {}))
  }
...
//   var disp = await spots[id].display(stat)
//   return disp[0]
// }
...
router.get('/spot/:id', async ctx => {
  ...
  try {
    var disp = await spots[id].display(ctx.state)
    ...
    ctx.body = make_body(
      ...
      disp[0],
      E("ul", {id: "actionList"}, disp[1] /* (await spotActions(id, ctx.state))*/.map(action => {
        console.log("action", action)
        return E("li", {},
                 E("form", {method: 'post', action: '/action/' + action.id},
                   E("button", {type: "submit", "class": "btn btn-primary"}, action.label)));
      })),
      E("ul", {id: "moveList"}, (await spotMoveOptions(id, ctx.state)).map(spot => {
