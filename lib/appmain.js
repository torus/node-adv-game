...
async function loadGame(gameId) {
  ...
  const charactersBox = []
  const sceneCmd = sceneCommands(charactersBox)
  ...
  const conv = converterFromCommands(sceneCmd)
  let sceneArray = doc.filter(e => e.scene).map(elem => conv(elem))
  console.log(sceneArray)

  const stageMap = {}
  const stageCmd = stageCommands(stageMap)
  const stgConv = converterFromCommands(stageCmd)
  doc.filter(e => e.stage).map(elem => stgConv(elem))
  console.log('stageMap', stageMap)

  let charactersElems = doc.filter(e => e.characters)
  charactersBox[0] = charactersElems.reduce((acc, cur) => {
    return merge(acc, cur.characters)
  }, {})
