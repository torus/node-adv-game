...
async function constructPage(ctx, title, ...content) {
  ...
  let e = E('html', {lang: "en"},
            E('head', {},
              ...
              E('meta', {name: 'viewport', content: 'user-scalable=no, width=device-width'}),
...
router.post('/games/:gameId', async ctx => {
  ...
  if (params.update) {
    ...
    if (semver.gt(ver, scenario.version) || !scenario.is_published && semver.eq(ver, scenario.version)) {
      ...
      while (true) {
        ...
        try {
          await pgClient.query(
            'UPDATE scenarios SET title = $2, description = $3, updated_at = $4, ' +
              'publisher = $5, version = $6, url = $7, is_published = $8 WHERE id = $1',
  ...
  if (params.published) {
    await pgClient.query(
      'UPDATE scenarios SET is_published = $1 WHERE id = $2', [JSON.parse(params.published), gameId])
  }
...
Speak.prototype.render = function(characters) {
  ...
  return E('div', {class: 'row adv-one-by-one'},
           E('div', {class: 'col-2', style: 'text-align: center'},
             (doc) => {
               const img = characters[this.character].image
               return img ? [
                 E('img', {src: img, style: 'max-width: 5ex'}),
                 E('br', {})
               ] : []
             },
             charName),
...
const UserParamDatabase = function(pgClient) {
  this.pgClient = pgClient
}

UserParamDatabase.prototype.set = async function(session, param, value) {
  await this.pgClient.query('BEGIN')
  await this.pgClient.query('DELETE FROM session_states WHERE session = $1 AND key = $2', [session.id, param])
  await this.pgClient.query('INSERT INTO session_states (session, key, value) VALUES ($1, $2, $3)',
                            [session.id, param, value])
  await this.pgClient.query('COMMIT')
}

UserParamDatabase.prototype.get = async function(session, param) {
  const res = await this.pgClient.query('SELECT value FROM session_states WHERE session = $1 AND key = $2',
                                        [session.id, param])
  if (res.rowCount > 0) {
...
const userParamDB = new UserParamDatabase(pgClient)
const {actionHandlers, convertScenario} = require('./scenario')(userParamDB)
...
async function loadGame(gameId) {
  ...
  const {stageMap, charMap, sceneArray} = convertScenario(doc)
  const stageDisplays = makeStages(stageMap, sceneArray)

  return gameCache[gameId] = {stages: stageMap
                              // , package: packageInfo
                              , characters: charMap
    ...
    ((scene, stage, prevDisplay) => {
      if (!prevDisplay) {
        ...
        stageDisplays[stage] = async session => {
          ...
          let curScene = await userParamDB.get(session, key)
...
async function allStageDisplays(gameId) {
  return (await loadGame(gameId)).displays
}
...
router.get('/stage/:sessId/:stageid', async ctx => {
  ...
  const displays = await allStageDisplays(gameId)
  const runtime = await displays[id](session)
        ...
        doc => {
          if (actions.length > 0) {
            return E("ul", {id: "actionList"}, actions.map(action => {
              return E("li", {},
                E('form', {onsubmit: 'doAction(' + action.id + '); return false'},
                  E('button', {id: 'action-button-' + action.id,
                               type: 'submit', class: 'btn btn-link'}, action.label))
               )
            }))(doc)
          } else {
            return E("ul", {id: "actionList"}, ' ')(doc)
          }
        },
        doc => {
          if (moves.length > 0) {
...
router.post('/action/:id', async ctx => {
  ...
  const {actionRuntime, session, stage, label} = await action(ctx)
...
console.log('stage', stage)
  ...
  actionRuntime.inst.forEach(inst => {
    ({
      desc: lines => {
        disp.push(textListToElements(lines))
      },
  ...
  if (moves.length == 0) {
    ...
    const displays = await allStageDisplays(gameId)
    const runtime = await displays[stage](session)
  ...
  ctx.body = {
    disp: disp.map(e => {
      return e(doc).toString();
    }),
    moves: moves.map(stage => {
      console.log('stage', stage)
      return E("li", {}, E("a", {href: ['/stage', session.id, stage.name].join('/')}, stage.label + 'へ行く'))(doc).toString()
    }),
