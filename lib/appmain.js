...
function sceneCommands(charMap) {
  ...
  sceneCmd.scene = (...args) => {
    ...
    self.display = async session => {
      ...
      return runtime
      // return [makeDisplayElements(disp, charMap), actions]
    }
  ...
  sceneCmd.speak = (actor, ...texts) => scene => async (runtime, session) => {
    ...
    // Runtime.prototype.addInstruction.call(runtime, 'speak', actor, texts)
    runtime.addInstruction('speak', actor, texts)
    // disp.push(new Speak(actor, texts))
  }
...
router.get('/stage/:sessId/:stageid', async ctx => {
  ...
  let stgs = await stages(gameId)
  console.log('stgs', stgs)
  let runtime = await (await stages(gameId))[id].display(session)
  ...
  ctx.body = await constructPage(
    ...
    // E('pre', {}, JSON.stringify(runtime, null, 3)),
    doc => {
      if (moves.length > 0) {
