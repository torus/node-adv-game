...
function sceneCommands() {
  ...
  sceneCmd.after = (...content) => scene => async (runtime, session) => async action => {
    ...
    actionHandlers[hndlid] = async ctx => {
      ...
      return {runtime: runtime, session: session, stage: scene.stage}
    }
  }
...
router.post('/action/:id', async ctx => {
  ...
  const {runtime, session, stage} = await action(ctx)
    ...
    E('div', {class: 'row adv-options'},
      E('div', {class: 'col'},
        E("a", {class: 'btn btn-primary',
                href: ['/stage', session.id, stage.name].join('/')}, '▼')))
  )
})
